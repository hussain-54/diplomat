...
import {
  CATEGORIES_SIDEBAR_ITEMS,
  isCategoriesNavActive,
} from "@/components/categories/nav";
import { getCategoriesLibraryCounts } from "@/lib/admin.functions";
...
const EXPAND_KEY = "newsroom-categories-nav-expanded";
...
export function CategoriesSidebarMenu({
  collapsed,
  roles,
}: {
  collapsed: boolean;
  ...
  const inCategories = location.pathname.startsWith("/admin/categories");

  const countsQ = useQuery({
    queryKey: ["categories-library-counts"],
    queryFn: getCategoriesLibraryCounts,
    staleTime: 30_000,
    enabled: hasPermission(roles, "categories:manage"),
  });

  const items = useMemo(
    () => CATEGORIES_SIDEBAR_ITEMS.filter((item) => hasPermission(roles, item.permission)),
    [roles],
  );

  const counts = countsQ.data ?? { all: 0, active: 0, hidden: 0, featured: 0 };
