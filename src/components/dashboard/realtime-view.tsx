...
import { ChartCard, DonutChart, LineTrendChart } from "@/components/dashboard/chart-card";
import {
  authorName,
  sectionName,
...
export function RealtimeView({
  ...
  const sources = [
    { label: "Direct", value: Math.max(liveVisitors, 1) },
    { label: "Search", value: Math.max(Math.round(liveVisitors * 0.7), 1) },
    { label: "Social", value: Math.max(Math.round(liveVisitors * 0.45), 1) },
    { label: "Referral", value: Math.max(Math.round(liveVisitors * 0.25), 1) },
  ];
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        ...
        action={
          <div className="flex items-center gap-2 border border-border px-3 py-1.5 text-[11px] text-muted-foreground">
            <span
              className={`h-2 w-2 rounded-full ${realtimeConnected ? "bg-cat-green" : "bg-gold"}`}
            />
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
        ...
        <MetricCard label="Breaking alerts" value={alerts.length} detail="Ticker desk" trend={alerts.length ? "up" : "neutral"} />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <ChartCard title="Live traffic pulse" description="Recent daily views" empty={!spark.some((r) => r.views)}>
          <LineTrendChart
            data={spark}
            dataKey="views"
            config={{ views: { label: "Views", color: "var(--color-crimson)" } }}
          />
        </ChartCard>
        <ChartCard title="Live traffic sources" description="Modeled concurrent mix">
          ...
          {!trending.length ? (
            ...
            <div className="divide-y divide-border">
              {trending.map((story, index) => (
                <div key={`${story.title}-${index}`} className="flex justify-between gap-3 px-5 py-3.5">
